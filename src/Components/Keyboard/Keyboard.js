...
//     return (
//         <div className={[classes.key, classes.backspace].join(' ')}>
        ...
        return (
            <Focusable onFocus={() => this.setState({active: true})}
                ...
                <div className={this.state.active ? [classes.focus, classes.key].join(' ') : [classes.key] + ' ' +(
                    typeof this.props.children !== 'string' ? classes.backspace : ''
                )}>
                    {this.props.children}
                </div>
            </Focusable>
...
export default class Keyboard extends Component {
    ...
    onBlur() {
        console.log('2', this.state.active);
    ...
    onEnterDown(event, navigation) {
        console.log('3', this.state.active);
        console.log(event, navigation);
        this.props.inputText(1)
        // navigation.forceFocus('sidebar');
    }
// <Focusable key={key} onFocus={() => this.onFocus()} onBlur={() => this.onBlur()}
// onEnterDown={(e, n) => this.onEnterDown(e, n)} navDefault>
// <div
// className={typeof key === 'string' ? classes.key : [classes.backspace, classes.key].join(' ')}>{key}</div>
// </Focusable>
    render() {
        return (
            <div className={classes.keyboard}>
