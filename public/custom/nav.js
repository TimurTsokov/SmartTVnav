!(function (window, undefined) {
    ...
    function Navigation() {
        ...
        function onKeyDown(e) {
            // var inputActive = document.activeElement === $('input')[0];
            // if (typeof Macid.getMACID !== 'undefined' || (document.getElementById('oipfConfiguration').localSystem !== undefined && !inputActive) || (navigator.userAgent.match(/Philips/) != null && !inputActive)) {
            //     e.preventDefault();
            // }
            var key,
                data = {},
         ...
         */
        function onClick() {
            navCur && navCur.is(':visible') && navCur.click();
            // navCur && navCur.click();
        }
        ...
        return {
            ...
            on: function (container, cur) {
                ...
                this.$container.on('mouseenter.nav', this.area_selector, function (e) {
                    if (!$(this).is(self.phantom_selector) && !$(this).is(self.ignore_mouse)) {
                ...
                if (!cur) {
                    cur = this.$container.find(this.area_selector).filter(':visible .nav-start')[0];
                    if (!cur) {
    ...
    $(function () {
        ...
        $(document.body).bind('nav_key:left nav_key:up nav_key:down', function (e) {
            var cur = nav.current(), $navs, n;
        ...
        $(document.body).bind('nav_key:right', function (e) {
            var cur = nav.current(), $navs, n;
            if (cur[0] != $('input')[0] || $('input').val() === '') {
                $navs = nav.$container.find(nav.area_selector).filter(':visible');
                n = nav.findNav(cur, e.keyName, $navs);
                n && nav.current(n);
            } else {
                var reqLength = $('input').val().length,
                    endPointer = $('input')[0].selectionEnd;
                if (endPointer < reqLength) {
                    $$nav.on($('.container'), $('input:first'));
                } else if (endPointer === reqLength) {
                    $navs = nav.$container.find(nav.area_selector).filter(':visible');
                    n = nav.findNav(cur, e.keyName, $navs);
                    n && nav.current(n);
                }
...
//     e.preventDefault();
//     if (evt.keyCode == KeyboardEvent.DOM_VK_BACK)
//     {
//         evt.preventDefault();
//     }
// });
